...
hungryWolf = monsters.mob("Hungry Wolf", 28, 3, 8, 10, 0, 4, "A ragged wolf appears out of the trees, drool slavering from its jaws.", "It growls in hunger. Perhaps prey has been hard to find these days.", "The wolf collapses, finally succumbing to its wounds and starvation.", "The wolf growls, but leaves you alone to find someone else.", 3, lootTable={"Rolls":1, "Wolf Fang":15, "Nothing":70, "Wolf Pelt":15})
...
def statAssign():
  ...
  if save == "y":
    ...
    stats = {"Level":1,"Strength":10, "Constitution":10, "Dexterity":10, "Agility":10, "Perception":10, "Intelligence":10, "Wisdom":10}
    sub_stats = {"Max HP":100, "Current HP":100, "Max MP":50, "Current MP":50, "Item Damage":0, "Crit Chance":0.0, "Learn Chance":0.0, "Damage-Upper":0, "Damage-Lower":0, "Defense":0} 
    statAssign()
"""
def save():
  print(gold + "Saved!")
...
def selector(): #Character selector for specific character attribute bonus thing.
  ...
  print(" Human" + gold +" (P-D)")
  print(cyan + " Elf" + gold +" (I-W)")
  print(cyan + " Dwarf" + gold +" (S-C)")
...
#I don't want to do this because it's so annoying ;-;
#UPDATE: It's DONE! Thank god. And I even managed to make some pretty well-optimized and efficient code.
def inventoryMenu():
...
def consumableMenu():
  ...
  listLegals = listConsumableItems[2:]
...
def monsterFight(mob):
  print("A " + mob.getMobName() + " appears before you.")
...
def tile9():
  ...
  userInput = checkIn(tilesetLegals, "str", ("What will you do? " + gold))
  if userInput == "search" or userInput == "look":
    if mobList > 0 and randint(1, 5) == 1:
      print("Y")
    elif mobList == 0:
...
stats = {"Level":1,"Strength":10, "Constitution":10, "Dexterity":10, "Agility":10, "Perception":10, "Intelligence":10, "Wisdom":10} #Main stats that're actually displayed.
